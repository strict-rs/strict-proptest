...
mod tests {
    ...
    impl CargoFingerprint {
        fn parse(json_text: &str) -> Result<Self, TestFailure> {
            let fingerprint_json: Value = ensure_ok(
    ...
    impl CargoArtifacts {
        ...
        fn current() -> Result<Self, TestFailure> {
            ...
            Ok(Self {
                deps_dir,
                fingerprint_dir,
                current_fingerprint: CargoFingerprint::parse(
                    &test_fingerprint,
                )?,
            })
        }
        ...
        fn resolve_artifact(
            ...
            let mut candidates = entries
                ...
                .filter_map(|artifact| {
                    let hash =
                        artifact_hash(&artifact, file_prefix, extension)?;
                    ...
                    let fingerprint =
                        CargoFingerprint::parse(&fingerprint_text).ok()?;
                    fingerprint
    ...
    fn path_to_str(path: &Path) -> Result<&str, TestFailure> {
        let path_text =
            ensure_some(path.to_str(), "the artifact path is valid UTF-8")?;
        ensure(
    ...
    -> Result<(), TestFailure> {
        let current = CargoFingerprint::parse(
            r#"{"rustc":1,"config":2,"features":"[]"}"#,
        )?;
