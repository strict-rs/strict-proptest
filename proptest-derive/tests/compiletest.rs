...
mod tests {
  ...
  impl CargoFingerprint {
    fn parse(json_text: &str) -> Result<Self, TestFailure> {
      let fingerprint_json: Value = ensure_ok(serde_json::from_str(json_text), "the Cargo fingerprint is valid JSON")?;
      Ok(Self {
        rustc:    json_u64(&fingerprint_json, "rustc", "the fingerprint carries a numeric rustc hash")?,
        config:   json_u64(&fingerprint_json, "config", "the fingerprint carries a numeric config hash")?,
  ...
  impl CargoArtifacts {
    ...
    fn current() -> Result<Self, TestFailure> {
      let current_exe = ensure_ok(env::current_exe(), "the current test binary path resolves")?;
      let deps_dir = ensure_some(current_exe.parent(), "the test binary sits in the target deps dir")?.to_owned();
      let fingerprint_dir = ensure_some(deps_dir.parent(), "the deps dir sits in the target profile dir")?.join(".fingerprint");

      let exe_hash = ensure_some(
        artifact_hash(&current_exe, "compiletest-", env::consts::EXE_EXTENSION),
        "the compiletest binary name carries a Cargo hash",
      )?;
      let test_fingerprint_path = fingerprint_dir
    ...
    fn resolve_artifact(
      ...
      let entries = ensure_ok(fs::read_dir(&self.deps_dir), "the target deps dir is readable")?;
  ...
  fn json_u64(json: &Value, field: &str, context: &'static str) -> Result<u64, TestFailure> {
  ...
  fn json_str<'a>(json: &'a Value, field: &str, context: &'static str) -> Result<&'a str, TestFailure> {
  ...
  fn path_to_str(path: &Path) -> Result<&str, TestFailure> {
    let path_text = ensure_some(path.to_str(), "the artifact path is valid UTF-8")?;
    ensure(
      !path_text.contains(char::is_whitespace),
  ...
  fn run_mode(src: &'static str, mode: &'static str) -> Result<(), TestFailure> {
    let mut config = ct::Config {
      mode: ensure_some(mode.parse().ok(), "the compiletest mode string parses")?,
  ...
  fn fingerprint_parsing_uses_typed_fields() -> Result<(), TestFailure> {
    let fingerprint = CargoFingerprint::parse(r#"{"rustc":17,"config":23,"features":"[\"default\", \"std\"]"}"#)?;

    ensure(fingerprint.rustc == 17, "the rustc hash is extracted as a number")?;
    ensure(fingerprint.config == 23, "the config hash is extracted as a number")?;
    ensure(fingerprint.features.contains("default"), "the decoded feature set carries default")?;
    ensure(fingerprint.features.contains("std"), "the decoded feature set carries std")
  }

  #[test]
  fn fingerprint_feature_matching_uses_exact_tokens() -> Result<(), TestFailure> {
    let current = CargoFingerprint::parse(r#"{"rustc":1,"config":2,"features":"[]"}"#)?;
    let candidate = CargoFingerprint::parse(r#"{"rustc":1,"config":2,"features":"[\"default-code-coverage\", \"std\"]"}"#)?;
