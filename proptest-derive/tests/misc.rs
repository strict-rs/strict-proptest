...
mod tests {
    ...
    */
    ...
    enum Bobby {
        ...
        #[proptest(
            params(Complex),
            strategy = "Just(Bobby::ParamStrategized(1))"
        )]
    ...
    impl Quux {
        const fn payload_score(&self) -> usize {
            match self {
                &Self::Bare(payload) | &Self::Braced { _foo: payload } => {
                    payload
                }
                &Self::Pair(payload, ref text)
                | &Self::PinnedPair(payload, ref text) => {
                    payload.saturating_add(text.len())
                }
            ...
            |value| {
                if let Some(payload) = value.payload() {
                    ensure_eq(
                        &payload,
                        &1,
                        "the strategy-built payload is one",
                    )?;
                }
                Ok(())
            },
    ...
    fn bobby_attributes_keep_payloads_reachable() -> TestResult {
        ensure_property(
            ...
            |value| match &value {
                &Bobby::Defaulted(payload) => ensure_eq(
                    &value.payload(),
                    &payload,
                    "the defaulted payload is reachable",
                ),
                &Bobby::Valued(_)
                | &Bobby::Strategized(_)
                | &Bobby::ParamValued(_)
    ...
    fn quux_attributes_keep_payloads_reachable() -> TestResult {
        ensure_property(
            ...
            |value| {
                ...
                match value {
                    Quux::Bare(payload) => ensure_eq(
                        &payload_score,
                        &payload,
                        "the bare payload is reachable",
                    ),
                    Quux::Pair(payload, text) => {
                        ...
                        ensure_eq(
                            &payload_score,
                            &expected,
                            "the tuple payloads are reachable",
                        )
                    }
