...
mod tests {
  ...
  fn ty1_always_v1() -> TestResult {
    ensure_property(&any::<Ty1>(), "skipped and uninhabited variants never generate", |sample| {
      ensure(matches!(sample, Ty1::V1), "only the inhabited, unskipped variant appears")
    })
  }

  #[test]
  fn ty_always_1_or_2() -> TestResult {
    ensure_property(&any::<Ty2>(), "multiple skipped variants never generate", |sample| {
      ensure(matches!(sample, Ty2::V1 | Ty2::V2), "only the unskipped variants appear")
    })
  }
