...
use strict_test_support::{ensure, ensure_ok, ensure_some};

use message_io::network::{SendStatus, ToRemoteAddr, Transport};
use system_under_test::{
    ClientDialer, ClientListener, Msg, ServerDialer, ServerListener,
};
...
trait EchoTransportExt {
    ...
    fn init_server<A>(
        self,
        addr: A,
    ) -> IoResult<(ServerDialer, ServerListener)>
    where
        A: ToSocketAddrs;
    ...
    fn init_client<A>(
        self,
        remote_addr: A,
    ) -> IoResult<(ClientListener, ClientDialer)>
    where
        A: ToRemoteAddr;
}
...
trait ClientDialerExt {
    ...
    fn msg_server(
        &mut self,
        msg: &str,
        implementation: SendImplementation,
    ) -> SendStatus;
}
...
impl ReferenceStateMachine for RefState {
    ...
    fn transitions(state: &Self::State) -> BoxedStrategy<Self::Transition> {
        use Transition::{
            ClientMsg, StartClient, StartServer, StopClient, StopServer,
        };
...
impl EchoServerTest {
    ...
    fn stop_all_clients(&mut self) -> Result<(), TestFailure> {
        let mut clients =
            take(&mut self.clients).into_iter().collect::<Vec<_>>();
    ...
    fn send_client_message(
        ...
        let send_status = client
            .dialer
            .msg_server(msg, SendImplementation::BUGGY_DEFAULT);
        ensure(
        ...
        let recv_msg =
            ensure_ok(received, "the server response is valid UTF-8")?;
        ensure(
            recv_msg == msg,
...
pub mod system_under_test {
    use message_io::network::{
        Endpoint, NetEvent, SendStatus, ToRemoteAddr, Transport,
    };
    ...
    use super::{
        ClientDialerExt, ClientListenerExt, EchoTransportExt,
        SendImplementation, ServerListenerExt,
    };
    ...
    impl EchoTransportExt for Transport {
            ...
            A: ToSocketAddrs,
        {
            ...
            let (_resource_id, address) =
                handler.network().listen(self, addr)?;
            ...
            A: ToRemoteAddr,
        {
            let (handler, listener) = node::split();
            let (server, _address) =
                handler.network().connect(self, remote_addr)?;
    ...
    impl ServerListenerExt for ServerListener {
        fn run_server(self) {
            let Self { listener, handler } = self;
