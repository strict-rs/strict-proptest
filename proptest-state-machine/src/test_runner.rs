...
#[cfg(feature = "std")]
use proptest::std_facade::fmt::{self, Debug};
use proptest::strict::{TestFailure, TestResult, strict_default_config};
...
pub trait StateMachineTest {
    ...
    fn test_sequential(
        ...
        if config.verbose >= INFO_LOG {
            ...
            emit_diagnostic_line(format_args!(
                "Running a test case with {trans_len} transitions."
            ));
        }
        #[cfg(not(feature = "std"))]
        drop(config);
...
mod tests {
    ...
    mod macro_test {
        ...
        impl crate::StateMachineTest for Test {
            ...
            fn init_test(
                _state: &<Self::Reference as crate::ReferenceStateMachine>::State,
            ) -> Self::SystemUnderTest {
            }
    ...
    mod strict_behavior {
        ...
        use proptest::strategy::{BoxedStrategy, Just, Strategy as _};
        use proptest::strict::{TestFailure, TestResult};
        use strict_test_support::{
            capture_ignored_test, ensure, ensure_eq, ensure_some,
        };
        ...
        -> Result<(), TestFailure> {
            let captured = capture_ignored_test(STACK_MACRO_CHILD)?;
                ...
                "the captured state-machine macro child passes",
            )?;
            ensure_eq(
                &captured.stderr,
                &String::new(),
