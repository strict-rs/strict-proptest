...
impl UseTracker {
  ...
  fn use_tyvar(&mut self, tyvar: impl Borrow<syn::Ident>) {
    ...
    if self.track
      && let Some(used) = self
        .used_map
        .iter_mut()
        .find_map(|entry| (entry.0 == *tyvar_ident).then_some(&mut entry.1))
    {
      *used = true;
    }
  }
  ...
  fn bound_all_params(&mut self, ctx: Ctx<'_>, for_used: &syn::TypeParamBound, for_not: &syn::TypeParamBound) -> DeriveResult<()> {
    self
      .used_map
      .iter()
      .map(|entry| &entry.1)
