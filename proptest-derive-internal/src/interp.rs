...
fn parse_lit_int(mut digits: &str) -> Option<u128> {
  ...
  loop {
    ...
    let digit = match current_byte {
      ...
      b'a'..=b'f' if base > 10 => 10_u128.saturating_add(u128::from(current_byte.wrapping_sub(b'a'))),
      b'A'..=b'F' if base > 10 => 10_u128.saturating_add(u128::from(current_byte.wrapping_sub(b'A'))),
      b'_' => {
        digits = digits.get(1..)?;
        continue;
...
fn eval_lit(lit: &syn::ExprLit) -> Option<u128> {
  match lit.lit {
    ...
    Lit::Str(_) | Lit::ByteStr(_) | Lit::CStr(_) | Lit::Char(_) | Lit::Float(_) | Lit::Bool(_) | _ => None,
  }
}
