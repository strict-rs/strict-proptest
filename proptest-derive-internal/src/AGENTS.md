...
Scope: `proptest-derive-internal/src/` — the `#[derive(Arbitrary)]` implementation pipeline. The proc-macro entry point itself lives in the sibling `proptest-derive` shim crate; for this crate's layout, features, visibility shape, and the nightly test commands see the crate `AGENTS.md` one directory up, and for workspace-wide conventions see the workspace-root `AGENTS.md`.

## The compile pipeline

`lib.rs` exposes one stable entry point: `derive_arbitrary(proc_macro2::TokenStream) -> proc_macro2::TokenStream`, which parses the input with `syn::parse2` (a parse failure — unreachable from rustc-driven derives — returns the error as `compile_error!` tokens rather than panicking) and hands the `DeriveInput` to `derive::impl_proptest_arbitrary`. The pipeline modules are documented `pub mod` boundaries with `pub(crate)`/private contents. The stages, in order:
...
- `value = "expr"` / `value(expr)` — a constant, non-shrinking strategy (`{ let value_fn: fn() -> _ = || expr; value_fn }`).
...
`Impl` = (type ident, `UseTracker`, `ImplParts = (Params, Strategy, Ctor)`). `into_tokens` emits a direct item-scope `impl ::proptest::arbitrary::Arbitrary for .. { type Parameters; type Strategy; fn arbitrary_with(..) }`; generated helper paths are fully qualified, so no anonymous const, local alias, or generated lint allowance is needed. `Params` (the `Parameters` IR) is the module's one genuinely-`pub` type: it derives `Debug`, and its `empty`/`len` helpers stay `pub(crate)`.
...
- The `Value` constructor expression is a typed-`let` coercion block, `{ let value_fn: fn() -> _ = || #expr; value_fn }` — the annotation performs the closure→fn-pointer coercion in any expression context without an `as` cast, and the arm stays a bare match arm (block-wrapping the `quote_append!` would leave a `()`-statement without `;`). The binding's ordinary name is collision-safe by scope — a `value_fn` named in the user's `value` expression resolves inside the closure initializer, where the `let` target is not yet in scope — and `tests/value.rs` in the shim crate pins exactly that with a colliding module-level `value_fn`.
- Span policy: the qualified `::proptest::...` helper paths are emitted with plain `quote!` (def-site spans); only the interpolated field type `#ty` carries its user span. That one span is what keeps a missing-`Arbitrary` error pointing at the offending field, while the def-site qualifiers stop consumer-side lints (`unused_qualifications`) from mis-attributing to user code — don't reintroduce `quote_spanned!` around these paths.
...
`tests.rs` is an inline `#[cfg(test)]` module of expansion tests using `test!` / `test_derive!` (adapted from `synstructure`): they run `impl_proptest_arbitrary` and compare its stringified tokens against an expected expansion. These tests are token-only on purpose; compile behavior for the real proc macro lives in the `proptest-derive` crate's `tests/` directory (run on nightly; see its `AGENTS.md`).
