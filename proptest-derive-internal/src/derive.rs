...
use crate::ast::Ctor;
use crate::ast::FromReg;
use crate::ast::Impl;
use crate::ast::ImplParts;
use crate::ast::MapClosure;
use crate::ast::Params;
use crate::ast::StratPair;
use crate::ast::Strategy;
use crate::ast::arbitrary_param;
use crate::ast::extract_all;
use crate::ast::extract_api;
use crate::ast::map_closure;
use crate::ast::pair_any;
use crate::ast::pair_any_with;
use crate::ast::pair_existential;
use crate::ast::pair_existential_self;
use crate::ast::pair_filter;
use crate::ast::pair_map;
use crate::ast::pair_oneof;
use crate::ast::pair_regex;
use crate::ast::pair_regex_self;
use crate::ast::pair_unit_self;
use crate::ast::pair_value;
...
pub(crate) fn impl_proptest_arbitrary(ast: DeriveInput) -> TokenStream {
  fn internal_error_tokens() -> TokenStream {
    let message = "[proptest_derive]: internal error, this is a bug! derive returned Fatal without a recorded diagnostic";
    quote::quote! {
...
fn derive_proptest_arbitrary(ctx: Ctx<'_>, ast: DeriveInput) -> DeriveResult<TokenStream> {
  ...
  let the_impl = match ast.data {
    // Deal with structs:
    syn::Data::Struct(struct_data) => derive_struct(ctx, DeriveData {
      tracker,
      attrs,
      ident: ast.ident,
    ...
    // Deal with enums:
    syn::Data::Enum(enum_data) => derive_enum(ctx, DeriveData {
      tracker,
      attrs,
      ident: ast.ident,
...
fn derive_struct(ctx: Ctx<'_>, mut ast: DeriveData<Vec<Field>>) -> DeriveResult<Impl> {
  ...
  let parts = if ast.body.is_empty() {
    ...
    let parts = if let Some(param_mode) = ast.attrs.params.into_top_params_mode() {
      // Parameters was set on the struct itself, the logic is simpler.
      add_top_params(
        param_mode,
        derive_product_has_params(ctx, &mut ast.tracker, error::STRUCT_FIELD, closure, ast.body)?,
      )
    } else {
      // We need considerably more complex logic.
      derive_product_no_params(ctx, &mut ast.tracker, ast.body, error::STRUCT_FIELD)?.finish(closure)
    };
...
fn add_top_filter(filter: Vec<Expr>, parts: ImplParts) -> ImplParts {
  ...
  let (filtered_strat, filtered_ctor) = add_filter_self(filter, (base_strat, base_ctor));
...
fn add_top_params(param_mode: TopParamsMode, (strat, ctor): StratPair) -> ImplParts {
  let params = Params::empty();
  match param_mode {
    TopParamsMode::Specified(params_ty) => {
      // We need to add `let params = _top;`.
      (params.with_type(*params_ty), strat, extract_api(ctor, FromReg::Top))
    }
...
fn derive_product_has_params(
  ...
  fields
    ...
    .try_fold(StratAcc::new(len), |acc, field| {
      ...
      let base_pair = product_handle_default_params(ut, ty, attrs.strategy);
...
fn product_handle_default_params(ut: &mut UseTracker, ty: Type, strategy: StratMode) -> StratPair {
  match strategy {
...
fn derive_product_no_params(ctx: Ctx<'_>, ut: &mut UseTracker, fields: Vec<Field>, item_kind: &str) -> DeriveResult<PartsAcc<Ctor>> {
  // Fold into an accumulator of the strategy types and the expressions
  ...
  fields.into_iter().try_fold(acc, |mut product_acc, field| {
    ...
    let pair = match attrs.params {
      ...
      ParamsMode::Passthrough => match attrs.strategy {
        // Specific strategy - use the given expr and erase the type:
        StratMode::Strategy(strat) => pair_existential(ty.clone(), strat),
        // Specific value - use the given expr:
        StratMode::Value(value_expr) => pair_value(ty.clone(), value_expr),
      ...
      ParamsMode::Default => product_handle_default_params(ut, ty.clone(), attrs.strategy),
      // params(<type>) set on the field:
      ParamsMode::Specified(params_ty) => {
        // We need to extract the param as the binding `params`:
        extract_nparam(&mut product_acc, *params_ty, match attrs.strategy {
          // Specific strategy - use the given expr and erase the type:
          StratMode::Strategy(strat) => pair_existential(ty.clone(), strat),
          // Specific value - use the given expr in a closure and erase:
          StratMode::Value(value_expr) => pair_value_exist(ty.clone(), value_expr),
          ...
          StratMode::Arbitrary => error::cant_set_param_but_not_strat(ctx, &ty, item_kind)?,
        })
      }
    };
...
fn extract_nparam<C>(acc: &mut PartsAcc<C>, params_ty: Type, (strat, ctor): StratPair) -> StratPair {
...
fn derive_enum(ctx: Ctx<'_>, mut ast: DeriveData<Vec<Variant>>) -> DeriveResult<Impl> {
  ...
  // The complexity of the logic depends mostly now on whether
  // parameters were set directly on the type or not.
  let enum_parts = if let Some(top_params) = ast.attrs.params.into_top_params_mode() {
    // The logic is much simpler in this branch.
    derive_enum_has_params(ctx, &mut ast.tracker, &ast.ident, ast.body, top_params)
  } else {
...
fn derive_enum_no_params(ctx: Ctx<'_>, ut: &mut UseTracker, _self: &Ident, variants: Vec<Variant>) -> DeriveResult<ImplParts> {
  // Initialize the accumulator:
  let mut acc = PartsAcc::new(variants.len());

  // Fold into the accumulator the strategies for each variant:
  for variant in variants {
    if let Some((weight, ident, fields, attrs)) = keep_inhabited_variant(ctx, variant)? {
      ...
      let (strat, ctor) = if fields.is_empty() {
        ...
        // Not a unit variant:
        derive_variant_with_fields(ctx, ut, path, attrs, fields.into_vec(), &mut acc)?
      };
...
fn derive_variant_with_fields<C>(
  ...
  let base_pair = match attrs.params {
    ...
    ParamsMode::Specified(params_ty) => extract_nparam(acc, *params_ty, match attrs.strategy {
      // Specific strategy - use the given expr and erase the type:
      StratMode::Strategy(strat) => {
        deny_all_attrs_on_fields(ctx, fields)?;
        pair_existential_self(strat)
...
fn variant_no_explicit_strategy<C>(
  ...
  let fields_acc = derive_product_no_params(ctx, ut, fields, error::ENUM_VARIANT_FIELD)?;
...
fn deny_all_attrs_on_fields(ctx: Ctx<'_>, fields: Vec<Field>) -> DeriveResult<()> {
  ...
  for variant in variants {
    ...
    if let Some((weight, ident, fields, attrs)) = parts {
      ...
      let (strat, ctor) = if fields.is_empty() {
        ...
        // Not a unit variant:
        let filter = attrs.filter.clone();
        add_filter_self(filter, variant_handle_default_params(ctx, ut, path, attrs, fields.into_vec())?)
      };
...
fn keep_inhabited_variant(ctx: Ctx<'_>, variant: Variant) -> DeriveResult<Option<VariantParts>> {
...
fn ensure_has_only_skip_attr(ctx: Ctx<'_>, attrs: &ParsedAttributes, item_kind: &str) {
...
fn pair_unit_variant(ctx: Ctx<'_>, attrs: &ParsedAttributes, v_path: &Path) -> StratPair {
