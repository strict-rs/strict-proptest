...
use self::FileFailurePersistence::Direct;
...
impl FailurePersistence for FileFailurePersistence {
  fn load_persisted_failures2(&self, source_file: Option<&'static str>) -> Vec<PersistedSeed> {
    let source = source_file.and_then(|source_path| absolutize_source_file(Path::new(source_path)));
    let resolved = self.resolve(source.as_deref());

    let persistence_path: Option<&PathBuf> = resolved.as_ref();
    let result: io::Result<Vec<PersistedSeed>> = persistence_path.map_or_else(
      || Ok(vec![]),
      |path| {
        ...
        io::BufReader::new(fs::File::open(path)?)
          ...
          .filter_map(|(lineno, line_result)| match line_result {
            ...
            Ok(seed_line_text) => parse_seed_line(&seed_line_text, path, lineno).map(Ok),
          })
          .collect()
      },
    );
  ...
  fn save_persisted_failure2(&mut self, source_file: Option<&'static str>, seed: PersistedSeed, shrunken_value: &dyn Debug) {
    ...
    if let Some(path) = resolved_path {
      ...
      match write_seed_data_to_file(&path, line.as_bytes()) {
        Err(error) => {
          diagnostics::emit(&RunnerDiagnostic::PersistenceAppendFailed {
            path,
            error,
          });
        }
...
fn absolutize_source_file_with_cwd(getcwd: impl FnOnce() -> io::Result<PathBuf>, source: &Path) -> Option<Cow<'_, Path>> {
  if source.is_absolute() {
    ...
    match getcwd() {
      Ok(mut cwd) => loop {
        ...
        if !cwd.pop() {
          diagnostics::emit(&RunnerDiagnostic::SourceNotAbsolutizable {
...
fn parse_seed_line(line: &str, path: &Path, lineno: usize) -> Option<PersistedSeed> {
  // Everything from the first '#' on is a comment:
  let seed_text = line.split_once('#').map_or(line, |(seed_text, _comment)| seed_text);
...
fn seed_line(seed: &PersistedSeed, shrunken_value: &dyn Debug) -> String {
  let comment = format!(" # shrinks to {shrunken_value:?}").replace(['\n', '\r'], " ");
  format!("{seed}{comment}\n")
}
...
fn write_seed_data_to_file(dst: &Path, seed_data: &[u8]) -> io::Result<bool> {
  ...
  match fs::OpenOptions::new().write(true).create_new(true).open(dst) {
    Ok(mut out) => {
      let mut record = Vec::with_capacity(FILE_HEADER.len().saturating_add(seed_data.len()));
...
fn crate_root_and_relative_source(source_path: &Path) -> Option<(PathBuf, PathBuf)> {
...
fn resolve_source_parallel(sibling: &'static str, source_path: &Path) -> Option<PathBuf> {
  let Some((dir, suffix)) = crate_root_and_relative_source(source_path) else {
  ...
  pub(super) fn resolve(&self, source: Option<&Path>) -> Option<PathBuf> {
    ...
    match *self {
      ...
      SourceParallel(sibling) => absolute_source.map_or_else(
        || {
          diagnostics::emit(&RunnerDiagnostic::SourceParallelSourceless);
          None
        },
      ...
      WithSource(extension) => absolute_source.map_or_else(
        ...
        |source_path| Some(set_extension_best_effort(Cow::into_owned(source_path), extension)),
      ),
...
mod tests {
  ...
  const PERSISTENCE_LOCATION_CHILD: &str =
