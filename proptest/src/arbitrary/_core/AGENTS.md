...
- `lift1!` (functor.rs) generates the higher-order `ArbitraryF1` impl so `proptest-derive` can lift a base strategy over a single-type-param container. `lift2!` preserves caller-supplied binary bounds and construction while sharing the `ArbitraryF2` method and boxing boundary for iterator adapters.
...
- `iter.rs` — the bulk of the directory: `Arbitrary` for iterator adapters. `wrap_ctor!` covers `Once`, `Repeat`, `Cycle`, `Enumerate`, `Fuse`, `Peekable`, `Rev`; `Empty`, `Cloned`, `Zip`, `Chain`, `StepBy`, and (via the local `usize_mod!` macro) `Skip`/`Take` get explicit `arbitrary!` + `lift1!`. `Cloned` additionally hand-rolls `ArbitraryF1` (its `Iterator<Item = &'a T>` lifetime can't go through `lift1!`); `Zip` and `Chain` use `lift2!` while retaining their distinct iterator bounds and construction. A `TODO` notes closure-carrying adapters (`Map`, `Filter`, `FlatMap`, `Scan`, …) are unsupported pending `CoArbitrary`.
...
- `num.rs` — `Wrapping`/`Saturating` via `wrap_ctor!`; the opaque `ParseFloatError`/`ParseIntError`/`TryFromIntError` built by triggering the real error (e.g. `"".parse::<f32>().unwrap_err()`); `FpCategory` as a five-way `prop_oneof!` of `Just`s over its variants.
- `option.rs` — `Option<A>` via `OptionStrategy<A::Strategy>`, parameterised by a `Probability` (chance of `Some`) packed with `A::Parameters`, built with `weighted(…)` from `crate::option`; `Probability` itself maps `(0.0..=1.0)` into the newtype (`MapInto<RangeInclusive<f64>, Self>`); `opt::IntoIter<A>` via `Option::into_iter`. `Option<core::convert::Infallible>` always generates `None`. This one impl also covers `string::ParseError` and current-nightly literal `!` through their type identities.
- `result.rs` — `Result<A, B>` via `MaybeOk<A::Strategy, B::Strategy>` (deemed the canonical choice), parameterised by `Probability` + both inner params via `maybe_ok_weighted`, with hand-rolled `ArbitraryF1`/`ArbitraryF2`; `IntoIter<A>` via `Result::into_iter`. `Result<_, core::convert::Infallible>` / `Result<core::convert::Infallible, _>` generate the single inhabited variant, including the matching `ArbitraryF1` lift for `Result<A, core::convert::Infallible>`. These impls also cover `string::ParseError` and current-nightly literal `!` through their type identities.

## Recurring patterns & gotchas

- **Uninhabited variants.** `Option`/`Result` over `core::convert::Infallible` are special-cased in every feature configuration: the empty side can't be generated, so the impl is either `None` or a map into the only inhabited variant. `string::ParseError` aliases `Infallible`, and current nightly aliases `Infallible` to `!`; do not add duplicate impls for those names. New "Option/Result of an empty type" cases belong here and must preserve distinct impl identities.
...
- **Literal-`!` tests:** native `Option`/`Result` generation and lifting tests use `#[cfg(all(feature = "unstable", not(feature = "alt-stable")))]` to preserve the stable compiler floor. The shared `Infallible` implementation and its tests are unconditional.
- **wasm32 carve-out:** the 128-bit `NonZero` types are excluded on `wasm32`.
- **no_std discipline.** Uninhabited container implementations import `Infallible` directly from `core::convert`. Allocating type families (`Vec`, `String`, `Box`, …) belong in the sibling `_alloc` tier, and `std`-only types in `_std`; shared allocated types and macros resolve through `crate::std_facade`.
