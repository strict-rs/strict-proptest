...
- `ops.rs` — range types: `RangeFull` (constant), `RangeFrom`/`RangeTo`/`RangeToInclusive` via `wrap_ctor!`, and `Range<A>`/`RangeInclusive<A>` which generate an unordered `(A, A)` pair and swap so `start <= end`. Under `unstable`, `CoroutineState<Y, R>` (a `Yielded`/`Complete` union) with a manual `ArbitraryF2`.
- `alloc.rs` — `#[cfg(feature = "unstable")]` only: the allocator API — `Global` and `AllocError` (constants) plus `Layout`, whose generator picks a power-of-two alignment (a `0..32` shift) and a size clamped to avoid the round-up overflow that `Layout::from_size_align` rejects. `System`/`CollectionAllocErr` are left commented out. (Its `//!` line mislabels the module as `std::hash` — a stale copy-paste; the content is the allocator API.)
...
- `#[cfg(feature = "unstable")]` — the whole `alloc.rs` module, `ToLowercase`/`ToUppercase`/`CharTryFromError` (`char.rs`), `CoroutineState` (`ops.rs`), the sub-`usize` atomics (`sync.rs`).
- `#[cfg(all(feature = "unstable", feature = "atomic64bit"))]` — `AtomicI64`/`AtomicU64` (`sync.rs`).
