...
- `lift1!(…)` — the companion `ArbitraryF1` higher-order impl (so `proptest-derive` can map over the container). Most files pair one `lift1!` per `arbitrary!`.
- `lift2!(…)` — shares the `ArbitraryF2` method and boxing boundary for maps and their owning iterators, preserving each caller's complete bounds and construction expression. `CoroutineState` retains its specialized manual implementation.
