  ...
  fn new_tree(&self, runner: &mut TestRunner) -> NewTree<Self> {
    ...
    Ok(ArrayValueTree {
      tree:          unarray::build_array_result(|_| {
        strategies
          .next()
          .ok_or_else(|| Reason::from("array strategy index out of range (internal invariant)"))?
          .new_tree(runner)
      })?,
      shrinker:      0,
      last_shrinker: None,
...
mod test {
  ...
  #[test]
  fn shrinks_fully_ltr() -> ProductShrinkCheck<[Range<i32>; 2]> {
    check_product_shrinking(&[0_i32..32, 0_i32..32], &mut TestRunner::deterministic(), |&pair| pair.into()).map(drop)
  }
