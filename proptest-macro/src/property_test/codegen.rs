...
pub(super) fn generate(item_fn: ItemFn, options: &Options) -> TokenStream {
  ...
  let arb_tokens = arbitrary::gen_arbitrary_impl(&argless_fn.sig.ident, &args, options);
  ...
  let new_body = test_body::body(&argless_fn.block, &args, &struct_and_arb, &argless_fn.sig.ident, options);
...
/// - otherwise, we use the name `arg<n>`, where `<n>` is the index of the argument (including ident
///   arguments)
///
...
mod tests {
  ...
  fn check_struct(
    ...
    let fixture_fn: ItemFn = ensure_ok(parse_str(fn_def), "fixture fn parses")?;
    let (stripped_fn, args) = ensure_stripped_args(fixture_fn)?;
    let tokens = generate_struct(&stripped_fn.sig.ident, &args);
    let parsed_struct: ItemStruct = ensure_ok(parse2(tokens), "generated struct parses")?;
    ...
    for field in parsed_struct.fields {
      ...
      rendered_fields.push(format!("{name}: {}", field.ty.to_token_stream()));
    }
  ...
  fn derives_debug() -> Result<(), TestFailure> {
    let fixture_fn: ItemFn = ensure_ok(parse_str("fn foo(x: i32) {}"), "fixture fn parses")?;
    let (stripped_fn, args) = ensure_stripped_args(fixture_fn)?;
  ...
  fn generates_correct_struct() -> Result<(), TestFailure> {
    ...
    check_struct("fn foo(a: i32, b: String) {}", "FooArgs", [("a", "i32"), ("b", "String")])
  }
  ...
  fn generates_arbitrary_impl() -> Result<(), TestFailure> {
    ...
    let (stripped_fn, args) = ensure_stripped_args(fixture_fn)?;
    let arb = arbitrary::gen_arbitrary_impl(&stripped_fn.sig.ident, &args, &Options::default());
...
mod snapshot_tests {
  ...
  macro_rules! snapshot_test {
    ($name:ident) => {
      snapshot_test!($name, $crate::property_test::options::Options::default());
    };
    ($name:ident, $options:expr) => {
      #[test]
      fn $name() -> Result<(), TestFailure> {
        const TEXT: &str = include_str!(concat!("codegen/test_data/", stringify!($name), ".rs"));

        let parsed = ensure_ok(parse_str(TEXT), "fixture source parses")?;
        let options = $options;
        let tokens = generate(parsed, &options);
        let file = ensure_ok(syn::parse_file(&tokens.to_string()), "generated code parses as a file")?;
  ...
  mod with_options {
    ...
    snapshot_test!(simple, Options {
      proptest_path: Some(ensure_ok(parse_str("::hello::world"), "custom proptest_path fixture parses",)?),
      ..Options::default()
    });
  }
}
