...
The runnable `dateparser_v1` and `dateparser_v2` examples use this typed runner. Their shared `examples/dateparser/tutorial.rs` module decodes fields with fallible `str::get` byte ranges and retains each fixed or generated input alongside its parser response. Version 1 therefore rejects invalid UTF-8 boundaries without the panic shown in the walkthrough above. Version 2 first rejects non-ASCII input and adds digit-shaped-date and round-trip properties. Both deliberately keep the one-digit month range `6..7`: the first example passes its limited checks, while the second returns a typed failure for the minimized round-trip counterexample `(0, 10, 1)`. Run them with `cargo run -p proptest --example dateparser_v1` and `cargo run -p proptest --example dateparser_v2`; the second command intentionally exits unsuccessfully. Neither executable enables regression-file persistence.
