...
**Optional features** plug into the runner and are gated in `proptest/Cargo.toml`: `fork` (process-isolate each case via `rusty-fork`), `timeout` (per-case time limits; requires `fork`), `bit-set` (bitset strategies), `unstable` (nightly-only language features), `handle-panics` (suppress intermediate panic spew during shrinking).
